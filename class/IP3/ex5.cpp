...
void hough_Lines(Mat src, vector<Vec2f>& s_lines, double rho, double theta, int thresh)
{
...
int main()
{
	Mat image = imread("images\\road_1.png", IMREAD_GRAYSCALE);
	...
	hough_Lines(canny, lines1, rho, theta, 50);
	...
	imshow("detected lines_OpenCV 함수", dst2);
	waitKey();
}
